...
function addAccessors($scope) {
  ...
  $scope.addLine = function () {
    ...
    canvas.add(new fabric.Line([50, 100, 200, 200], {
      left: coord.left,
      top: coord.top,
      stroke: '#000' + getRandomColor()
    }));
  };
  $scope.addPolygon = function () {
    var coord = getRandomLeftTop();
    this.canvas.add(new fabric.Polygon([{x: 185, y: 0}, {x: 250, y: 100}, {x: 385, y: 170}, {
      x: 0,
      y: 245
    }], {left: coord.left, top: coord.top, fill: '#FFF' + getRandomColor(), stroke: '#000', opacity: 0.2}));
  };
  ...
  var addShape = function (shapeName) {
    ...
    fabric.loadSVGFromURL('../assets/' + shapeName + '.svg', function (objects, options) {
      ...
      loadedObject.set({left: coord.left, top: coord.top, angle: getRandomInt(-10, 10)}).setCoords();
  ...
  $scope.shadowify = function () {
    ...
    if (obj.shadow) {
      ...
      obj.setShadow({color: 'rgba(0,0,0,0.3)', blur: 10, offsetX: 10, offsetY: 10});
    }
    canvas.renderAll();
  };
  $scope.gradientify = function () {
    ...
    obj.setGradient('fill', {
      ...
      colorStops: {0: '#' + getRandomColor(), 1: '#' + getRandomColor()}
    });
    canvas.renderAll();
  };
  ...
  function addTexts() {
    ...
    var iText2 = new fabric.IText('', {
      ...
      fill: '#333',
      styles: {
        0: {0: {fill: 'red'}, 1: {fill: 'red'}, 2: {fill: 'red'}},
        2: {0: {fill: 'blue'}, 1: {fill: 'blue'}, 2: {fill: 'blue'}, 3: {fill: 'blue'}}
      }
    });
  ...
  function initDiamondPatternBrush() {
    ...
    $scope.diamondPatternBrush.getPatternSrc = function () {
      ...
      var rect = new fabric.Rect({width: squareWidth, height: squareWidth, angle: 45, fill: this.color});
